...
function drawFootnote(doc: jsPDF, project: Project, catalog: Catalog, pricing: boolean, pageW: number, pageH: number): void {
  ...
  doc.text(
    `${finishLabel} · Box: ${project.settings.boxMaterial === "PLY" ? "Plywood" : "Particle Board"}. Dimensions nominal; verify field measurements before ordering. * = SKU is single-box only at the supplier.${pricing ? ` Pricing per OPPEIN ${catalog._meta.pricing_year ?? 2025} list.` : ""}`,
    24,
    y + 10,
    { maxWidth: pageW - 48 },
  );
