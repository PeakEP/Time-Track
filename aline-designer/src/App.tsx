...
export default function App() {
  ...
  const [paletteWidth, setPaletteWidth] = useState(() => readWidth("jmrc.aline.paletteW", 280));
  const [scheduleWidth, setScheduleWidth] = useState(() => readWidth("jmrc.aline.scheduleW", 340));

  useEffect(() => {
    writeWidth("jmrc.aline.paletteW", paletteWidth);
  }, [paletteWidth]);
  useEffect(() => {
    writeWidth("jmrc.aline.scheduleW", scheduleWidth);
  }, [scheduleWidth]);
  ...
  return (
    <div className="app-shell">
      ...
      <footer className="app-footer">
        <span>
          {catalog
            ? `Catalog: ALINE ${catalog._meta.pricing_year ?? 2025} · ${catalog.products.length} SKUs · ${catalog.finishes.length} finishes`
            : "Loading catalog…"}
        </span>
        <span>
