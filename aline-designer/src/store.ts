...
// localStorage keys are namespaced under "aline" so the OPPEIN designer's drafts
// (which use "jmrc.cabinet.*") and the Aline designer's drafts can coexist on
// the same domain without overwriting each other.
const AUTOSAVE_KEY = "jmrc.aline.draft.v1";
const AUTOSAVE_DEBOUNCE_MS = 600;
const SHOW_PRICING_KEY = "jmrc.aline.showPricing.v1";
const SHOW_DIMS_KEY = "jmrc.aline.showDimensions.v1";
...
export function loadCatalog(): Promise<Catalog> {
  ...
  return fetch(`${base}aline-catalog.json`).then((r) => {
